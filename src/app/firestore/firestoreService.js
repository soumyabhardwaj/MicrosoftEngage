...
/*whenever the event is selected and if we want to update our event */
    return db.collection('events').doc(event.id).update(event)
}

export const deleteEventInFirestore = eventId => {
    return db.collection('events').doc(eventId).delete()
    /*as the name suggests it will delete the event */
}
...
export const addUserAttendance = async event => {
    const user = firebase.auth().currentUser
    const eventdata= await db.collection('events').doc(event.id).get();
  
    if(eventdata.data().attendeeIds.length<30)
    return db.collection('events').doc(event.id).update({
        attendees: firebase.firestore.FieldValue.arrayUnion({
            id: user.uid,
            displayName: user.displayName,
